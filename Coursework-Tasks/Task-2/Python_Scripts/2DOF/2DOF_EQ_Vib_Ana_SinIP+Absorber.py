# Mech Vib Tsak 2 (2DOF, Earthquake Vibration + ABSORBER, Analytic Sol) #
...
F1o = w_eq**2*m1*Yo
F2o = w_eq**2*m2*Yo
Fo = sp.Matrix([F1o, 0, F2o, 0]) # Equivalent Seismic Forces
...
#-----------------------------------------------------------
Xeq1_history = [] # Wind 1st response history
N1 = Z(w_eq)
for i in range(0,4):
    N1[i,0] = Fo[i]
Xeq1 = det(N1) / det(Z(w_eq))

Xeqa_history = [] # Absorber response history
Na = Z(w_eq)
for i in range(0,4):
    Na[i,1] = Fo[i]
Xeqa = det(Na) / det(Z(w_eq))


Xeq2_history = [] # Wind 2nd response history
N2 = Z(w_eq)
for i in range(0,4):
    N2[i,2] = Fo[i]
Xeq2 = det(N2) / det(Z(w_eq))

Xeqb_history = [] # Absorber response history
Nb = Z(w_eq)
for i in range(0,4):
    Nb[i,3] = Fo[i]
Xeqb = det(Nb) / det(Z(w_eq))
#-----------------------------------------------------------
...
#--------------------------------------------------
    ...
    x_eq1 = abs(Xeq1)*sin(w_eq*t-phase(Xeq1)) # Steady state response from wind
    Xeq1_history.append(x_eq1*1e3)

    #* Absorber a steady state response:
    x_eqa = abs(Xeqa)*sin(w_eq*t-phase(Xeqa)) # Steady state response from wind
    Xeqa_history.append(x_eqa*1e3)

    #* 2nd steady state response:
    x_eq2 = abs(Xeq2)*sin(w_eq*t-phase(Xeq2)) # Steady state response from wind        
    Xeq2_history.append(x_eq2*1e3)
    
    #* Absorber b steady state response:
    x_eqb = abs(Xeqb)*sin(w_eq*t-phase(Xeqb)) # Steady state response from wind
    Xeqb_history.append(x_eqb*1e3)
#--------------------------------------------------
...
Xeqb_w_history = np.zeros((len(w_list),1)) # Earhquacke absolute response history
for j in range(0,len(w_list)):
        ...
        Xeq1_w_history[j] = abs(Xeq1)
        Xeqa_w_history[j] = abs(Xeqa)
        Xeq2_w_history[j] = abs(Xeq2)
        Xeqb_w_history[j] = abs(Xeqb)
...
graphEQ.legend(['x_eq1(t) [m]', 'x_eq2(t) [m]', 'y(t) [m]'], fontsize=13, loc='lower right')
graphEQ.grid(True)
...
graphEQ2.legend(['q1(t) [m]', 'q2(t) [m]', 'y(t) [m]'], fontsize=13, loc='lower right')
graphEQ2.grid(True)
...
graphEQ3.plot(w_list, Xeq1_w_history, color='pink', linewidth=10)
graphEQ3.plot(w_list, Xeq2_w_history, color='lightgreen', linewidth=5)
graphEQ3.plot(w_list, Xeqa_w_history, color='darkred', linewidth=2)
graphEQ3.plot(w_list, Xeqb_w_history, color='darkgreen', linewidth=2)
...
graphEQ3.legend(['Xeq1(t) [m]', 'Xeq2(t) [m]', 'Xeqa(t) [m]', 'Xeqb(t) [m]'], fontsize=13, loc='upper right')
graphEQ3.grid(True)
